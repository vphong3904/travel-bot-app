...
async def suggest_kb_draft(question: str) -> dict:
    """
    Gọi Gemini soạn draft KB. Raise RuntimeError khi Gemini lỗi/không cấu hình
    (route chuyển thành HTTPException 502).
    """
    from google.genai import types as genai_types

    from app.core.config import settings
    from app.services.gemini_optimizer import call_with_retry
    ...
    async def _call():
        return await asyncio.wait_for(
            client.aio.models.generate_content(
                ...
                contents=_PROMPT_TEMPLATE.format(question=question.strip()[:500]),
